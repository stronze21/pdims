<div class="flex flex-col px-5 mx-auto max-w-screen">
    <x-mary-header title="Delivery View" separator progress-indicator>
        ...
        <x-slot:actions>
            @if ($details && $details->status == 'pending')
                <x-mary-button label="Save & Lock" icon="o-lock-closed" class="btn-sm btn-warning shadow-md"
                    wire:click="$set('lockModal', true)" />
                @if ($details->delivery_type != 'RIS')
                    <x-mary-button label="Add Item" icon="o-plus" class="btn-sm btn-primary shadow-md"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <div class="flex">
                        ...
                        <span
                            class="font-bold text-sm uppercase">{{ $details->supplier ? $details->supplier->suppname : '' }}</span>
                    </div>
                    <div class="flex">
                ...
                <div class="space-y-2">
                    <div class="flex">
                        <span
                            class="w-36 text-gray-500 text-sm">{{ $details->charge_code == 'DRUMAK' ? 'Reference' : 'Purchase Order' }}
                            #:</span>
                    ...
                    <div class="flex">
                        ...
                        <span
                            class="badge {{ $details->status == 'pending' ? 'badge-ghost' : 'badge-success' }} badge-sm font-bold uppercase">
                            {{ $details->status }}
                        </span>
                    </div>
                </div>
                ...
                <table class="table table-sm table-zebra">
                    <thead class="bg-gradient-to-r from-slate-700 via-slate-600 to-slate-700">
                        <tr>
                            ...
                            <th class="text-white text-xs font-bold uppercase tracking-wide py-4 px-4 text-right">QTY
                            </th>
                            <th class="text-white text-xs font-bold uppercase tracking-wide py-4 px-4 text-right">Unit
                                Cost</th>
                            <th class="text-white text-xs font-bold uppercase tracking-wide py-4 px-4 text-right">Retail
                                Price</th>
                            <th class="text-white text-xs font-bold uppercase tracking-wide py-4 px-4 text-right">Total
                                Amount</th>
                            @if ($details->status == 'pending')
                                <th class="text-white text-xs font-bold uppercase tracking-wide py-4 px-4 text-center">
                                    Actions</th>
                            @endif
                        </tr>
                    </thead>
                                ...
                                <td class="py-3 px-4 text-right text-xs font-mono">
                                    @if ($item->current_price && $item->current_price->has_compounding)
                                        <span class="tooltip tooltip-left"
                                            data-tip="Includes {{ number_format($item->current_price->compounding_fee, 2) }} compounding fee">
                                            <x-mary-icon name="o-information-circle"
                                                class="w-3 h-3 inline text-blue-500" />
                                        </span>
                                    @endif
        ...
        <x-mary-form wire:submit="add_item">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <x-mary-input label="Unit Cost" wire:model="unit_price" type="number" step="0.01"
                    min="0" icon="o-currency-dollar" required />
            </div>
    ...
    {{-- Edit Item Modal --}}
    <x-mary-modal wire:model="editItemModal" title="Edit Item: {{ $editItemName }}" class="backdrop-blur"
        box-class="max-w-xl">
        <x-mary-form wire:submit="edit_item">
            <x-mary-input label="Lot No" wire:model="edit_lot_no" icon="o-hashtag" required />
            <x-mary-input label="Expiry Date" wire:model="edit_expiry_date" type="date" icon="o-calendar"
                required />

            <x-mary-checkbox label="Highly Specialised Drugs" wire:model.live="edit_has_compounding" />

            @if ($edit_has_compounding)
                <x-mary-input label="Compounding Fee" wire:model="edit_compounding_fee" type="number"
                    step="0.01" icon="o-currency-dollar" required />
            @endif
