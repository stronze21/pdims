<div>
    <x-mary-header title="Emergency Room Prescriptions" separator />
    ...
    <div class="flex justify-center gap-2 mb-4">
        ...
        <a href="{{ route('rx.er') }}" class="btn btn-primary btn-sm" wire:navigate>
    ...
    <x-mary-card>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
                ...
                <div class="flex gap-2 flex-wrap">
                    <button wire:click="setRxTagFilter('all')"
                        class="btn btn-xs {{ $rx_tag_filter === 'all' ? 'btn-primary' : 'btn-ghost' }}">
                        All
                    </button>
                    <button wire:click="setRxTagFilter('basic')"
                        class="btn btn-xs {{ $rx_tag_filter === 'basic' ? 'btn-accent' : 'btn-ghost' }}">
                        BASIC
                    </button>
                    <button wire:click="setRxTagFilter('g24')"
                        class="btn btn-xs {{ $rx_tag_filter === 'g24' ? 'btn-error' : 'btn-ghost' }}">
                        G24
                    </button>
                    <button wire:click="setRxTagFilter('or')"
                        class="btn btn-xs {{ $rx_tag_filter === 'or' ? 'btn-secondary' : 'btn-ghost' }}">
                        OR
                    </button>
                </div>
            </div>
            ...
            <div>
                ...
                <x-mary-input type="date" wire:model.live="filter_date" :max="date('Y-m-d')" class="input-sm" />
            </div>
        </div>
            ...
            <table class="table table-zebra table-sm">
                ...
                <tbody>
                    @forelse ($prescriptions as $index => $rx)
                        ...
                        @if ($showRow)
                            <tr wire:key="er-{{ $rx->enccode }}" wire:click="viewEncounter('{{ $rx->enccode }}')"
                                ...
                                <td class="whitespace-nowrap">
                                    ...
                                    <div class="text-xs opacity-70">
                                        {{ \Carbon\Carbon::parse($rx->ertime)->format('g:i A') }}
                                    </div>
                                </td>
                                <td class="whitespace-nowrap">
                                ...
                                <td class="whitespace-nowrap">
                                    @if ($rx->empprefix || $rx->lastname)
                                        <div class="text-sm font-medium">
                                            {{ $rx->empprefix }} {{ $rx->lastname }}, {{ $rx->firstname }}
                                            {{ $rx->middlename }}
                                        </div>
                                    @endif
                                    <div class="text-xs opacity-70">{{ $rx->tsdesc }}</div>
                                </td>
                                <td>
                                    @php
                                        $class = match ($rx->mssikey ?? null) {
