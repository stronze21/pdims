    ...
    <x-mary-header title="Stock Card" separator progress-indicator>
        ...
        <x-slot:actions>
            <div class="flex items-end space-x-2">
                <x-mary-button icon="o-arrow-down-tray" class="btn-sm btn-info" onclick="ExportToExcel('xlsx')" tooltip="Export" />
                <x-mary-button icon="o-printer" class="btn-sm btn-primary" onclick="printMe()" tooltip="Print" />
                @can('admin')
                    <x-mary-button icon="o-arrow-path" class="btn-sm btn-warning" wire:click="initCard" spinner="initCard" tooltip="Init Card" />
                @endcan
            </div>
        </x-slot:actions>
    </x-mary-header>
            ...
            <table class="table table-sm w-full" id="table">
                <thead class="bg-gradient-to-r from-slate-700 via-slate-600 to-slate-700 sticky top-0 z-10">
                    ...
                    <tr>
                        ...
                        <th class="text-white text-xs font-bold uppercase tracking-wide py-3 px-4">Beginning Balance</th>
                ...
                <tbody>
                    @forelse ($cards as $card)
                        ...
                        <tr class="hover:bg-blue-50 transition-colors border-b border-gray-100">
                            ...
                            <td class="py-3 px-4 text-xs text-right font-semibold">{{ number_format($card->reference) }}</td>
                            <td class="py-3 px-4 text-xs text-right text-green-600">{{ number_format($card->rec) }}</td>
                            <td class="py-3 px-4 text-xs text-right text-red-600">{{ number_format($card->iss) }}</td>
                            <td class="py-3 px-4 text-xs text-right text-orange-600">{{ number_format($card->pullout_qty) }}</td>
