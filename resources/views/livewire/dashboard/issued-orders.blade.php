        ...
        <div>
            <div class="text-sm breadcrumbs">
                <ul>
                    ...
                    <li>Issued / Dispensed Orders</li>
                </ul>
            </div>
            <h1 class="text-2xl font-bold">Issued / Dispensed Orders</h1>
            <p class="text-sm opacity-60">Drugs that have been issued and dispensed to patients</p>
        </div>
            ...
            <div class="flex items-center gap-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-success">{{ number_format(count($records)) }}</p>
                    </div>
                </div>
                <div class="border-l border-base-200 pl-6">
                    <p class="text-sm opacity-60">Total Qty Issued</p>
                    <p class="text-xl font-bold">{{ number_format($records->sum('qtyissued')) }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body p-0">
                        ...
                        <tbody>
                            @foreach ($records as $index => $record)
                                <tr class="hover">
                                    ...
                                    <td class="text-right font-semibold">{{ number_format($record->qtyissued) }}</td>
                                    ...
                                    <td class="whitespace-nowrap">{{ $record->date_issued ? \Carbon\Carbon::parse($record->date_issued)->format('M d, Y h:i A') : '-' }}</td>
            ...
            @else
                <div class="flex flex-col items-center justify-center py-12 opacity-40">
                    ...
                    <p>No issued orders found for the selected period</p>
                </div>
            @endif
        </div>
    </div>
